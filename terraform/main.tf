...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "f6374f9d-b6e2-4d1c-96a2-8254223c54c6"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.6437378797604457"
...
resource "aws_db_subnet_group" "default" {
  name       = "manufacturingonaspdotnet-db-subnet"
...
#  name        = "manufacturingonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application manufacturingonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for manufacturingonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "manufacturingonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "manufacturingonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_manufacturingonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "manufacturingonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/manufacturing-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/manufacturingonaspdotnet theharbormaster/manufacturing-onaspdotnet:latest"
    ]
  }
}
